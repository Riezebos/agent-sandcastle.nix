{lib}: let
  ...
  in "02:00:${octet 0}:${octet 2}:${octet 4}:${octet 6}";

  tapForName = name: "tap-${builtins.substring 0 11 (builtins.hashString "sha256" name)}";

  shareTagForName = prefix: name: "${prefix}-${builtins.substring 0 16 (builtins.hashString "sha256" name)}";
...
in {
  ...
  mkSandbox = {
    ...
    agentStartScript =
      if agentCommand == []
      then "${pkgs.coreutils}/bin/sleep infinity"
      else "${pkgs.writeShellScript "agent-sandcastle-agent-command" ''
        set -eu
